...
const UpdateDeposits = () => {
  ...
  const perPage = 6
  const totalPages = Math.ceil(allDeposits.length / perPage)
  const startIndex = (currentPage - 1) * perPage
  const currentAllDeposits = allDeposits.slice(startIndex, startIndex + perPage)
  ...
  const HandleSearch = () => {
    ...
    if (!search) {
      ...
      setCurrentPage(1)
      const showSearch = altDeposits.filter(item => item.depositUser.username.includes(search.toLowerCase()) || item.depositUser.email.includes(search.toLowerCase()) || moment(item.createdAt).format('DD-MM-yyyy').includes(search) || item.amount.toString().includes(search) || item.crypto.toLowerCase().includes(search.toLowerCase()) || item.status.includes(search.toLowerCase()) || item.gen_id.includes(search))
                      ...
                      <div key={i} className='w-full h-fit relative sha rounded-lg text-black font-medium'>
                        <div className='p-4 bg-semi-white text-sm rounded-t-lg flex justify-between gap-4'>
                          ...
                          <div className='flex gap-4 items-center'>
                            ...
                            <div className='hover:text-[#9f7ae7] cursor-pointer bg-white py-0.5 rounded-[3px]' onClick={() => { setSingleDeposit(item); setModal(true) }}><BsThreeDotsVertical /></div>
                          </div>
                        </div>
